 ...
 walk:
  while (pc != end) {
    ...
    case RE_SAVE:
      pc++;
      break;  /* zero-width */
    case RE_BOL: case RE_EOL: case RE_BOT: case RE_EOT: case RE_EOTNL: case RE_GPOS:
...
compile_atom(re_compiler *c)
{
  ...
  case '\\':
    ...
    if (ch >= '1' && ch <= '9') {
      ...
      /* spinel: \G, the position the search started from -- the string
         start for a plain #match, the end of the previous match for a
         scan/gsub step (spinel #3637). Only the backtracking engine keeps
         that position. */
      next_char(c);
      emit(c, RE_GPOS, 0, 0);
               ...
               mrb_bool binary)
{
  ...
  if (pat->anchor != RE_ANCHOR_BOT) {
    ...
    for (uint32_t i = 0; i < code_len && plen < 255; i++) {
      ...
      if (op == RE_SAVE ||
          op == RE_BOL || op == RE_EOL || op == RE_BOT || op == RE_EOT || op == RE_GPOS ||
