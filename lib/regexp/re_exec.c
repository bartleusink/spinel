...
#ifdef SP_THREADS
  /* spinel: the claim has to be atomic where threads are real, since a
     pattern is shared by every thread matching against it (spinel #4082), so
     it is made here and given back below if the search cannot run */
...
typedef struct {
  ...
  int pass_seq;
  mrb_bool binary;
  const char *gpos;   /* spinel: where this search started, for \G (RE_GPOS) */
...
bt_match(bt_state *m, const char *sp, uint32_t pc)
{
  ...
  for (;;) {
    ...
    case RE_GPOS:
      /* spinel: \G, only the position this search started from */
      if (sp != m->gpos) goto fail;
      pc++;
      break;
