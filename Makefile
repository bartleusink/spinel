...
infer-test: $(SPINEL) $(SP_RT_LIB)
	...
	test "$$(grep -c ': warning: ' "$$tmp/ww.err")" = 3 && grep -q '^test/infer/emit_types_fields.rb:6:13: warning: parameter `o` of `dist2` widened to untyped' "$$tmp/ww.err" && grep -q '^test/infer/emit_types_fields.rb:11:1: warning: the return of `widen` widened' "$$tmp/ww.err" || { echo "infer-test: FAIL (--warn-widen: one warning per widened slot, at the slot, 1-based column)"; cat "$$tmp/ww.err"; ok=0; }; \
	...
	$(SPINEL) test/infer/emit_types_fields.rb --warn-widen --no-line-map -c -o "$$tmp/ww2.c" 2>&1 >/dev/null | grep -q '^test/infer/emit_types_fields.rb:6:13: warning' || { echo "infer-test: FAIL (--warn-widen places past --no-line-map)"; ok=0; }; \
