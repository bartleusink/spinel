...
infer-test: $(SPINEL) $(SP_RT_LIB)
	...
	grep -q '^spinel: test/infer/why_widen_chain.rb:24:3: note: returned `return nil if n > 5…` is String, where a `return` gives nil (two kinds meet: untyped)' "$$tmp/wwc.err" && grep -q '^spinel: test/infer/why_widen_chain.rb:24:10: note: and `nil` is nil' "$$tmp/wwc.err" || { echo "infer-test: FAIL (--warn-widen: why: a return that met two kinds names the return of the other kind)"; cat "$$tmp/wwc.err"; ok=0; }; \
	$(SPINEL) test/infer/why_widen_chain.rb -c --no-line-map -o "$$tmp/wwc2.c" 2>&1 >/dev/null | grep -q ': note: ' && { echo "infer-test: FAIL (a plain compile derives no origin)"; ok=0; }; \
